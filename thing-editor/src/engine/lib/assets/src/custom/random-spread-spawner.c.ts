...
export default class SpreadSpawner extends Container {
    ...
    spreadX = 0;  // Максимальное смещение по X (±spreadX)

    @editable({ min: 0, step: 1 })
    spreadY = 0;  // Максимальное смещение по Y (±spreadY)
    ...
    protected getLocalSpawnPosition(resultPoint: Point) {
        ...
        // Добавляем случайное смещение по X и Y
        if (this.spreadX > 0) {
            resultPoint.x += (Math.random() - 0.5) * this.spreadX * 2;
        }
        if (this.spreadY > 0) {
            resultPoint.y += (Math.random() - 0.5) * this.spreadY * 2;
        }
    }
...
(SpreadSpawner.prototype.enable as SelectableProperty).___EDITOR_isGoodForCallbackChooser = true;
(SpreadSpawner.prototype.disable as SelectableProperty).___EDITOR_isGoodForCallbackChooser = true;
(SpreadSpawner.prototype.spawn as SelectableProperty).___EDITOR_isGoodForCallbackChooser = true;
(SpreadSpawner.prototype.setSpeed as SelectableProperty).___EDITOR_isGoodForCallbackChooser = true;

SpreadSpawner.__EDITOR_icon = 'tree/spawner';
