...
const videoEditorProps = { className: 'asset-editor' };

let assetNameProps = {
    className: 'selectable-text',
    title: CTRL_READABLE + '+click to copy video`s name',
    onMouseDown: copyTextByClick
};

const VideoEditor = (props: EditablePropertyEditorProps): ComponentChild => {
    const file = fs.getFileByAssetName(props.value, AssetType.VIDEO);
    return R.div(videoEditorProps,
        R.btn(props.value ? R.span(assetNameProps, props.value) : '. . .', () => {
            game.editor.chooseAsset(AssetType.VIDEO, 'Select "' + props.field.name, props.value, undefined, props.field.filterAssets).then((selectedVideo) => {
                if (selectedVideo) {
                    props.onChange(selectedVideo);
...
VideoEditor.parser = (val: string) => {
    return val || null;
};

VideoEditor.contextMenuInjection = (contextMenu: ContextMenuItem[], _field: EditablePropertyDesc, _clickedValue: any, _value: any) => {
    if (_clickedValue) {
        contextMenu.splice(contextMenu.indexOf(null) + 1, 0, {
            name: 'Reveal In Explorer',
            onClick: () => {
                const file = fs.getFileByAssetName(_clickedValue, AssetType.VIDEO);
                if (file) {
